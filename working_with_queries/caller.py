...
def find_authors_nationalities():
    return '\n'.join(f'{str(b)} is {b.nationality}' for b in Author.objects.exclude(nationality=None))
...
def order_books_by_year():
    ...
    return '\n'.join(f'{b.publication_year} year: {str(b)}' for b in
...
def filter_authors_by_birth_year(*range):
    result = []

    for author in Author.objects.filter(birth_date__year__range=range).order_by('-birth_date'):
        result.append(f'{str(author.birth_date)}: {str(author)}')
